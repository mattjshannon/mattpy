...
# Load data:
# DATA_DIR = 'input/'
DATA_DIR = 'input_non_aliphatic/'
# OUTPUT_DIR = 'output/'
OUTPUT_DIR = 'output_non_aliphatic/'
...
for index, filename in enumerate(FILE_LIST):
    ...
    result = fullspec.measure_all_bands(output_dir=OUTPUT_DIR)

    results.append(result)
    # TODO: save model results if we're going that way.


np.savetxt(OUTPUT_DIR + "results.txt", np.array(results),
           header='Source, 7.7 flux (W/m^2), 7.7 flux error (W/m^2)',
           delimiter=',', fmt='%s')
