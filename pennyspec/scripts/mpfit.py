...
Perform Levenberg-Marquardt least-squares minimization, based on MINPACK-1.
 ...
 Sergey Koposov converted the Mark's Python version from Numeric to numpy
    ...
    http://code.google.com/p/astrolibpy/source/browse/trunk/
...
Keywords:
 ...
 iterfunct:
        ...
        If the fit is to be stopped for any reason, then iterfunct should
        return a
        a status value between -15 and -1.  Otherwise it should return None
        (e.g. no return statement) or 0.
        In principle, iterfunct should probably not modify the parameter
        values, because it may interfere with the algorithm's stability.
 ...
 maxiter:
        The maximum number of iterations to perform.  If the number is
        exceeded,
 ...
 xtol:
        ...
        accordingly set to 2 or 3).  Therefore, xtol measures the
        relative error
 ...
 .perror
        ...
        *If* you can assume that the true reduced chi-squared value is unity --
        ...
        if (self.damp != 0) and (autoderivative == 0):
            self.errmsg = 'ERROR: keywords DAMP and AUTODERIVATIVE ' \
        ...
        if parinfo is not None:
            if not isinstance(parinfo, list):
                ...
                if not isinstance(parinfo[0], dict):
                    self.errmsg = 'ERROR: PARINFO must be a list of ' \
                                  'dictionaries.'
                    return
            if ((xall is not None) and (len(xall) != len(parinfo))):
                self.errmsg = 'ERROR: number of elements in PARINFO and P ' \
                              'must agree'
                return
        ...
        if xall is None:
            ...
            if xall is None:
                self.errmsg = 'ERROR: either P or PARINFO(*)["value"] ' \
                              'must be supplied.'
                return
        ...
        self.params = xall.copy()  # self.params is the set of
        ...
        if (limited is not None) and (limits is not None):
            ...
            if numpy.any((limited[:, 0] & limited[:, 1]) &
                         ...
                         (pfixed == 0)):
                self.errmsg = \
                    'ERROR: PARINFO parameter limits are not consistent'
                return
        ...
        while(1):
            ...
            if self.fnorm != 0:
                for j in range(n):
                    ...
                    if wa2[lx] != 0:
                        ...
                        gnorm = numpy.max([gnorm, numpy.abs(sum0 / wa2[lx])])
            ...
            while(1):
                ...
                if self.status < 0:
                    self.errmsg = \
                        'WARNING: premature termination by "' + fcn + '"'
                    return
                ...
                if ~numpy.all(numpy.isfinite(wa1) & numpy.isfinite(wa2) &
                    ...
                    self.errmsg = ('''ERROR: parameter or function value(s) '
                                   'have become infinite; check model function'
                                   ' for over- 'and underflow''')
                    self.status = -16
                    break
                # wh = where(finite(wa1) EQ 0 OR finite(wa2) EQ 0 OR
                # finite(x) EQ 0, ct)
        ...
        for j in range(minmn):
            if pivot != 0:
                ...
                if ct > 0:
                    ...
                    # Exchange rows via the pivot only.
                    # Avoid actually exchanging
                    # the rows, in case there is lots of memory transfer.  The
                    # exchange occurs later, within the body of MPFIT,
                    # after the
